...
using WhatsappBolado.Models;
using Xamarin.Forms;

namespace WhatsappBolado.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        public MainViewModel()
        {
         ...
         .WithUrl($"http://whatsappboladao.gearhostpreview.com/chathub")
         .Build();

            hubConnection.On<string>("JoinChat", (user) =>
            {
                Messages.Add(new MessageModel() { User = Name, Message = $"{user} entrou no chat", IsSystemMessage = true });
            });

            hubConnection.On<string>("LeaveChat", (user) =>
            {
                Messages.Add(new MessageModel() { User = Name, Message = $"{user} saiu do chat", IsSystemMessage = true });
            });
